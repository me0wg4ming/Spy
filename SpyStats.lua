...
function SpyStats:Filter()
    ...
    for _, unit in ipairs(units.recent) do
        ...
        -- Fix: unit kann String oder Table sein
        local unitName, unitGuild, unitKos, unitWins, unitLoses, unitReason
        if type(unit) == "string" then
            unitName = unit
        else
            unitName = unit.name
            unitGuild = unit.guild
            unitKos = unit.kos
            unitWins = unit.wins
            unitLoses = unit.loses
        ...
        if filter == "" then
            ...
            if unitGuild and type(unitGuild) == "string" then
                local lowerGuild = string.lower(unitGuild)
                local lowerFilter = string.lower(filter)
                guildMatch = string.find(lowerGuild, lowerFilter, 1, true) == 1
            end
        end

        if (nameMatch or guildMatch)
            and (not filterkos or unitKos)
            and (not filterpvp or ((unitWins and unitWins > 0) or (unitLoses and unitLoses > 0)))
            and (not filterreason or unitReason)
        then
